...
class SimulationOrchestrator:
    ...
    def save_state(
        ...
        if selection_method == "percentage":
            ...
            basis = parameters.get("selection_basis", "random")
            if basis == "random":
                target_agents = random.sample(agent_list, min(count, self.num_agents))
            else:
                warnings.warn(f"Unsupported basis '{basis}'. Default random.", RuntimeWarning)
                target_agents = random.sample(agent_list, min(count, self.num_agents))
        else:
