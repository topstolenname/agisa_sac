...
diff --git a/src/agisa_sac/orchestrator.py b/src/agisa_sac/orchestrator.py
index eb236de903efbef8dc5deb9636b7f7192239f0b4..1983896dd623b522c401aff6a23326641bde9ff9 100644
--- a/src/agisa_sac/orchestrator.py
+++ b/src/agisa_sac/orchestrator.py
@@ -1,136 +1,288 @@
 import numpy as np
 import time
 import json
...
+    # Optional GPU/embedding dependencies
+    try:
+        import cupy as cp
+        HAS_CUPY = True
+    except ImportError:
...
+        HAS_CUPY = False
+
+    HAS_SENTENCE_TRANSFORMER = False  # Placeholder for future integration
...
+                try:
+                    multiplier = random.uniform(heuristic_mult_range[0], heuristic_mult_range[1])
...
+                except Exception as e:
+                    warnings.warn(
+                        f"Stress failed for {agent.agent_id}: {e}", RuntimeWarning
+                    )
...
+                "cupy_random_state": cp.random.get_random_state().get_state()
+                if HAS_CUPY and self.config.get('use_gpu', False)
+                else None,
+            }
...
+                state = pickle.load(f)
+
+            loaded_framework_version = state.get("framework_version")
+            if loaded_framework_version != FRAMEWORK_VERSION:
+                warnings.warn(
+                    f"Version mismatch: loading '{loaded_framework_version}' into '{FRAMEWORK_VERSION}'.",
+                    category=UserWarning,
+                )
+
+            self.config = state.get("config", self.config)
+            self.num_agents = self.config.get('num_agents', 100)
...
+            if HAS_CUPY and "cupy_random_state" in state and state["cupy_random_state"] is not None:
+                cupy_state = cp.random.get_random_state()
+                cupy_state.set_state(state["cupy_random_state"])
+
+            agents_state_data = state.get("agents_state", {})
+            self.agents = {}
+            print(f"Reconstructing {len(agents_state_data)} agents...")
+            for agent_id, agent_data in agents_state_data.items():
+                if agent_id not in self.agent_ids:
+                    warnings.warn(
+                        f"Skipping agent {agent_id} from save (not in current config)."
+                    )
+                    continue
+                try:
+                    self.agents[agent_id] = EnhancedAgent.from_dict(
+                        agent_data, self.message_bus, strict_validation=False
+                    )
+                except Exception as e:
+                    warnings.warn(f"Failed to load agent {agent_id}: {e}", RuntimeWarning)
+
...
+
+            self.chronicler = ResonanceChronicler()
+            if "chronicler_state" in state:
+                self.chronicler.load_state(state["chronicler_state"])
+
+            self.tda_tracker = PersistentHomologyTracker(
+                max_dimension=self.config.get('tda_max_dimension', 1)
+            )
+            if "tda_tracker_state" in state and self.tda_tracker:
+                self.tda_tracker.load_state(state["tda_tracker_state"])
+
+            self.analyzer = AgentStateAnalyzer(self.agents)
+            self.is_running = False
+            print(f"State loaded. Resuming at epoch {self.current_epoch + 1}.")
+            return True
+        except FileNotFoundError:
+            warnings.warn(
+                f"Load failed: File not found at {filepath}", category=RuntimeWarning
+            )
+            return False
+        except Exception as e:
+            warnings.warn(f"Load failed: {e}", category=RuntimeWarning)
+            import traceback
+            traceback.print_exc()
+            return False
