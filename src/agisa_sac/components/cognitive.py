...
class CognitiveDiversityEngine:
    ...
    def to_dict(self, history_limit: int = 10) -> Dict:
        ...
        return { "version": FRAMEWORK_VERSION, "personality": self.personality, "heuristics": self.heuristics.tolist(),
                 "learning_rate": self.learning_rate, "stability_factor": self.stability_factor,
                 "cognitive_state": self.cognitive_state.tolist(), "decision_history_summary": self.decision_history[-history_limit:] }
    ...
    def from_dict(cls, data: Dict[str, Any], agent_id: str, memory_layer: 'MemoryContinuumLayer', message_bus: Optional['MessageBus']) -> 'CognitiveDiversityEngine':
        ...
        if loaded_version != FRAMEWORK_VERSION: warnings.warn(f"Agent {agent_id}: Loading cognitive v '{loaded_version}' into v '{FRAMEWORK_VERSION}'.", UserWarning)
        # Need to pass memory_layer and message_bus which are runtime objects
        instance = cls( agent_id=agent_id, personality=data['personality'], memory_layer=memory_layer, message_bus=message_bus )
