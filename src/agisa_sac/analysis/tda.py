...
#   AGI-SAC_Clean/src/agisa_sac/analysis/tda.py
#
...
# Calculate the path to the clean repository's tda.py
_base_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../../'))
_clean_path = os.path.join(_base_path, 'AGI-SAC_Clean/src/agisa_sac/analysis/tda.py')

# Load the module directly from the clean repository using importlib
_spec = importlib.util.spec_from_file_location("_clean_tda", _clean_path)
if _spec is None or _spec.loader is None:
    raise ImportError(f"Could not load tda module from clean repository: {_clean_path}")
...
PersistentHomologyTracker = _clean_module.PersistentHomologyTracker

__all__ = ["PersistentHomologyTracker"]
