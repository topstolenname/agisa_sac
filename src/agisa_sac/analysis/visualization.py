...
def plot_persistence_diagram(
    ...
    **kwargs,
    ...
    if diagram is None or diagram.ndim != 2 or diagram.shape[1] != 2 or diagram.shape[0] == 0:
...
def plot_persistence_barcode(
    ...
    **kwargs,
    ...
    inf_death_val = max_finite_death + 0.1 * (max_finite_death - np.min(plot_diagram[:, 0]) + 1e-6)
    inf_indices = np.isinf(plot_diagram[:, 1])
    plot_diagram[inf_indices, 1] = inf_death_val
...
def plot_metric_comparison(
    ...
    if total_plots == 0:
        ...
        return
    fig, axes = plt.subplots(total_plots, 1, figsize=figsize, sharex=True, squeeze=False)
    ...
    for metric_key in metrics_to_plot:
        ...
        valid_epochs = [
            e for i, e in enumerate(epochs) if values[i] is not None and np.isfinite(values[i])
        ]
        valid_values = [v for v in values if v is not None and np.isfinite(v)]
        if valid_values:
            ax.plot(valid_epochs, valid_values, marker=".", linestyle="-", label=metric_key)
    ...
    if tda_metric_history and tda_metrics_to_plot:
        ...
        for metric_key in tda_metrics_to_plot:
            ax = axes[plot_idx, 0]
            values = [
                tda_metric_history[e].get(metric_key) for e in tda_epochs if e in tda_metric_history
            ]
            valid_epochs = [
                e
                for e in tda_epochs
