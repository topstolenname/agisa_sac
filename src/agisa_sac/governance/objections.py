...
class ObjectionTracker:
    ...
    def file_objection(
        ...
        if len(recent) >= self.rate_limit_count:
            raise ValueError(
                f"Objection rate limit exceeded for party {party_id}. "
                f"Limit: {self.rate_limit_count} per {self.rate_limit_window}s"
            )
        ...
        objection_id = (
            f"obj-{decision_id}-{party_id}-{int(now)}-{count + 1}-{unique_suffix}"
        )
