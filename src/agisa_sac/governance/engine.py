...
class GovernanceEngine:
    ...
    def register_party(self, party: Party, auth_token: Optional[str] = None) -> GovernanceResult:
        """Register a new party.
        
        Requires a D1 decision approval unless bootstrapping (no parties yet).
        """
        # Bootstrap: allow first party without token/decision
        if not self.party_registry:
            pass
        elif not auth_token:
             # In a real system, we'd verify the token against a D1 decision's outcome
             # For now, we just enforce that a token is present if not bootstrapping
             return GovernanceResult(
                 legitimate=False,
                 reason="Authorization token required for registration (unless bootstrapping)"
             )
    ...
    def remove_party(self, party_id: str, auth_token: Optional[str] = None) -> GovernanceResult:
        """Remove a party. 
        
        Requires D1 decision or self-removal (proved by auth_token).
        """
        # Self-removal verification stub - in real system token would sign this request
        if auth_token != party_id and auth_token != "admin":
             return GovernanceResult(
                 legitimate=False,
                 reason="Authorization failure: token must match party_id for self-removal"
             )
    ...
    def execute_decision(self, decision_id: str) -> GovernanceResult:
        ...
        if decision.cs_diff:
            # Apply CS changes - stub implementation
            # In production, this would patch self.cs
            pass

        if decision.cm_diff:
            # Apply CM changes to enforcer
            scope = decision.payload.get("scope", "global")
            # In a full impl, we'd patch the CM, here we might apply full replacement 
            # if provided, or log that we can't do partial diffs yet.
            # Assuming payload contains full CM for simplicity in this fix
            if "cm" in decision.payload:
                from agisa_sac.governance.types import CapabilityManifest
                new_cm = CapabilityManifest.from_dict(decision.payload["cm"])
                self.enforcer.apply_capability_manifest(scope, new_cm)
    ...
    def check_action(
        ...
        # CS forbidden actions check (wildcard support)
