...
def add_concord_subparser(subparsers: Any) -> None:
    ...
    add_p.add_argument("--auth-token", dest="auth_token", default=None, help="Authorization token (required except bootstrap)")

    rm_p = party_sub.add_parser("remove", help="Remove a party")
    rm_p.add_argument("--id", dest="party_id", required=True)
    rm_p.add_argument("--auth-token", dest="auth_token", default=None, help="Authorization token (required)")
...
def _handle_party(args: argparse.Namespace, engine: Any) -> int:
    ...
    if party_cmd == "add":
        ...
        result = engine.register_party(party, auth_token=args.auth_token)
        _print_result(result)
        return 0 if result.legitimate else 1
    elif party_cmd == "remove":
        result = engine.remove_party(args.party_id, auth_token=args.auth_token)
        _print_result(result)
        return 0 if result.legitimate else 1
    elif party_cmd == "list":
