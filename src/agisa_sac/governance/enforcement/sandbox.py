        ...
        if state.cm.compute_quota:
            ...
            if (
                ...
                self._log_enforcement(
                    ...
                    {
                        "action": action,
                        "reason": "compute_quota_exceeded",
                        "metric": "steps",
                        "used": used.get("steps", 0),
                        "limit": quota["max_steps"],
                    },
                )
            ...
            if (
                quota.get("max_time_seconds")
                and used.get("time_seconds", 0) >= quota["max_time_seconds"]
            ):
                self._log_enforcement(
                    scope,
                    "action_denied",
                    {
                        ...
                        "reason": "compute_quota_exceeded",
                        "metric": "time_seconds",
                        "used": used.get("time_seconds", 0),
                        "limit": quota["max_time_seconds"],
                    },
                )
                return False, "compute quota exceeded (time_seconds)"
