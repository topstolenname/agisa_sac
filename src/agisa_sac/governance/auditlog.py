...
class AuditEntry:
    ...
    def compute_hash(self) -> str:
        ...
        # Serialize data first to ensure deterministic hashing of nested dicts
        serialized_data = json.dumps(self.data, sort_keys=True, default=str)
...
def test_audit_log_determinism():
    """Verify that audit log hashing is deterministic for unsorted dicts."""
    entry1 = AuditEntry(
        entry_id="e1",
    ...
    entry2 = AuditEntry(
        entry_id="e1",
        data={"a": 1, "c": { "x": 1, "y": 2}, "b": 2},
        previous_hash="abc"
    )
    # Timestamps might differ if default factory used, but here we set IDs manually
    # Force timestamps to match for test
    entry1.timestamp = 1000.0
    entry2.timestamp = 1000.0
