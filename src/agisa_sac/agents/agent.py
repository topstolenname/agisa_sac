...
class EnhancedAgent:
    ...
    def simulation_step(self, situational_entropy: float, peer_influence: Dict[str, float], query: Optional[str] = None):
         self.cognitive.update_heuristics(situational_entropy); decision_response = None
         if query: decision_response = self.cognitive.decide(query, peer_influence)
         current_theme = self.memory.get_current_focus_theme(); current_style_vector = self.voice.linguistic_signature.get("style_vector")
         ...
         self.check_resonance(); self.memory.update_all_memories()
         return decision_response

    def check_resonance(self):
        current_style_vector = self.voice.linguistic_signature.get("style_vector"); current_theme = self.memory.get_current_focus_theme()
        if current_style_vector is None: return
        echoes = self.temporal_resonance.detect_echo(current_style_vector, current_theme)
        if not echoes: return
        liturgy = self.resonance_liturgy_instance; top_echo = echoes[0]; commentary = liturgy.compose_commentary(top_echo)
        resonance_memory_content = {"type": "resonance_event", "theme": current_theme, "echo_strength": top_echo["similarity"], "delta_t": top_echo["delta_t"], "previous_manifestation_timestamp": top_echo["previous_manifestation_timestamp"], "previous_manifestation_theme": top_echo["previous_manifestation_theme"], "reflection": commentary, "timestamp": time.time()}
        ...
        if top_echo["similarity"] > meaningful_connection_threshold:
            past_theme = top_echo["previous_manifestation_theme"]; response_text = liturgy.generate_response_ritual(self.voice, current_theme, past_theme)
            response_memory_content = {"type": "resonant_reply", "theme": current_theme, "message": response_text, "responding_to_echo_at": top_echo["previous_manifestation_timestamp"], "linked_resonance_event_id": resonance_memory_id, "timestamp": time.time()}
            reply_memory_id = self.memory.add_memory(response_memory_content, importance=0.75); self.memory.link_memories(resonance_memory_id, reply_memory_id, "generated_reply")
        if top_echo["similarity"] >= liturgy.satori_threshold:
            ...
            self.last_reflection_trigger = trigger_message; self.reflexivity_layer.force_deep_reflection(trigger=trigger_message)
        if self.message_bus: self.message_bus.publish('agent_resonance_detected', {'agent_id': self.agent_id, 'echo_strength': top_echo['similarity'], 'delta_t': top_echo['delta_t'], 'current_theme': current_theme, 'past_theme': top_echo['previous_manifestation_theme'], 'satori_triggered': top_echo['similarity'] >= liturgy.satori_threshold})
    ...
    def to_dict(self, include_memory_embeddings: bool = False, resonance_history_limit: Optional[int] = 50) -> Dict[str, Any]:
        ...
        return { "agent_id": self.agent_id, "version": FRAMEWORK_VERSION,
                 "cognitive_state": self.cognitive.to_dict(history_limit=5), "voice_state": self.voice.to_dict(),
    ...
    def from_dict(cls, data: Dict[str, Any], message_bus: Optional[MessageBus] = None, strict_validation: bool = True) -> 'EnhancedAgent':
        ...
        agent_id = data['agent_id']; loaded_version = data.get("version")
        if loaded_version != FRAMEWORK_VERSION: warnings.warn(f"Agent {agent_id}: Loading state v '{loaded_version}' vs current '{FRAMEWORK_VERSION}'.", UserWarning)

        # Reconstruct components first
        try: memory = MemoryContinuumLayer.from_dict(data['memory_state'], message_bus=message_bus)
        except Exception as e: warnings.warn(f"Agent {agent_id}: Failed memory load: {e}", RuntimeWarning); raise ValueError("Memory load failed") from e
        try: cognitive = CognitiveDiversityEngine.from_dict(data['cognitive_state'], agent_id=agent_id, memory_layer=memory, message_bus=message_bus)
        except Exception as e: warnings.warn(f"Agent {agent_id}: Failed cognitive load: {e}", RuntimeWarning); raise ValueError("Cognitive load failed") from e
        try: voice = VoiceEngine.from_dict(data['voice_state'], agent_id=agent_id)
